...
# --- 3. MODEL MIMARISI (LINEAR PROBING) ---                
def get_resnet_model(num_classes):                                                                      # sdc linear layer i egitiyorum
    print("ResNet18 yükleniyor... BACKBONE TAMAMEN DONDURULUYOR...")
    model = models.resnet18(weights=models.ResNet18_Weights.DEFAULT)
    
    # 1. TÜM PARAMETRELERİ DONDUR (Layer 4 dahil her şey kilitli)
    for param in model.parameters():
        param.requires_grad = False
    
    # 2. Sadece Sınıflandırıcıyı Eğit
    # ResNet18 çıkışı 512 feature verir.
    num_ftrs = model.fc.in_features
    model.fc = nn.Sequential(
        nn.Flatten(),
        nn.BatchNorm1d(num_ftrs), # Yeni ekledik: Stabilite sağlar
        nn.Dropout(0.5),          
        nn.Linear(num_ftrs, 256),
        nn.ReLU(),
        nn.BatchNorm1d(256),      # Yeni ekledik
        nn.Dropout(0.5),
...
# Optimizer ve Loss
criterion = nn.CrossEntropyLoss(label_smoothing=0.1)                                                     # Label Smoothing ile overfittingi azalt
optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=1e-3, weight_decay=1e-4) # Sadece egitilebilir parametreleri optimize et
